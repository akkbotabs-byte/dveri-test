/* Карточки моделей (каталог и «Похожие модели»): кликабельные свотчи.
   ...
   сбрасывается к состоянию фильтра. Без JS кнопки-свотчи стоят
...
window.DSDCards = (function () {
  ...
  /* Вариант показа: кадр цвета; у цвета без своего кадра — дефолт */
  function variantFor(card, slug) {
    var src = slug && card.getAttribute('data-img-' + slug);
    if (src) {
      return { src: src, alt: card.getAttribute('data-alt-' + slug) };
    }
