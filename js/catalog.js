/* Каталог: клиентские фильтры (ПГ/ПО, отделка, цвет), сортировка,
   ...
   При ровно одном выбранном «Типе полотна» карточки моделей с кадрами
   обоих типов показывают кадр этого glazing (данные data-img-pg/po,
   в комбинации с цветом — data-img-<цвет>--pg/po; багрепорт владелицы
   19.08.2026: Палитра 11-4 в списке глухих выглядела остеклённой).
...
(function () {
  ...
  function apply() {
    ...
    if (cardsApi) {
      ...
      /* Тип полотна: при ровно одном выбранном карточки с кадрами обоих
         типов показывают кадр этого glazing (сброс/оба — приоритет
         у цвета/дефолта); перекраску делает paint ниже */
      if (cardsApi.setFilterGlazing) {
        cardsApi.setFilterGlazing(glazing.length === 1 ? glazing[0] : null);
      }
    }
